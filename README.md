...
두 모드 모두 옵션형(위자드)과 대화형(챗)에서 사용할 수 있으며, 각 방식에 맞는 프롬프트가 적용됩니다.

#### 옵션형 vs 대화형 — 프롬프트 구성 차이

싱글/멀티 에이전트 모드에서 옵션형과 대화형은 **동일한 에이전트 파이프라인**을 사용하지만, 각 에이전트의 **시스템 프롬프트가 다릅니다**:

| 에이전트 | 옵션형 (A/B/C 생성) | 대화형 (AI 챗) |
|----------|---------------------|----------------|
| **Generator** | `GENERATOR_SYSTEM_PROMPT` — JSON 형식으로 A/B/C 3종 메시지 생성 | `build_chat_system_prompt()` — 자연어 대화로 메시지 작성 도움 |
| **Reviewer** | `REVIEWER_SYSTEM_PROMPT` — A/B/C JSON 검토 후 개선된 JSON 응답 | `CHAT_REVIEWER_SYSTEM_PROMPT` — 자연어 응답 검토 후 자연어로 개선 |
| **Spam Checker** | `SPAM_CHECKER_SYSTEM_PROMPT` — KISA 기준 스팸 분석 (공통) | 동일 (공통) |

> 대화형 모드에서 Reviewer가 JSON으로 응답하면 채팅 UI에 깨진 텍스트가 표시됩니다.
> 이를 방지하기 위해 대화형 전용 Reviewer 프롬프트(`CHAT_REVIEWER_SYSTEM_PROMPT`)가 별도로 존재하며,
> "반드시 자연스러운 한국어 텍스트로만 응답하세요"라는 지시를 포함합니다.
