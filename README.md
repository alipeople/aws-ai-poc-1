...
AWS Bedrock LLM 기반 AI 메시지 작성 POC. 옵션형(5단계 위자드)과 대화형(AI 챗) 두 가지 모드를 지원하며, 실시간 SSE 스트리밍 응답을 제공합니다.
...
- **KISA 스팸 분석**: 에이전트 모드와 **독립적으로** ON/OFF 가능. 싱글/멀티 모드 모두에서 KISA 기준 스팸 분류·광고표기 준수·위험 키워드 분석
...
- **WHAT → WHO 흐름**: 페이지 제목 "✍️ 메시지 작성하기", CTA "👥 누구에게 보낼까요?" (다음 단계 WHO로 연결 예정)
