...
7. LMS, MMS, 알림톡 등 90자 이상 메시지에서는 반드시 줄바꿈을 사용하세요. JSON text 필드 안에서 줄바꿈은 \\n 문자로 표현합니다. 인사말, 핵심 혜택, 상세 정보, CTA, 수신거부를 각각 별도 줄로 구분하세요.
8. 이모티콘, 이모지, 특수 기호(♥★♪◆●▶■△♡☆ 등)를 사용하지 마세요. 순수 텍스트로만 작성하세요.
9. 광고표기 규칙 (한국 광고성 메시지 법적 필수사항):
